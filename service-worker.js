const CACHE_NAME='informe-eds-v15';
...
function forceHeader(html){
  ...
  // Remove every occurrence of the old visible wording from HTML text/attributes.
  html=html.replace(/Mantenimiento eléctrico y mecánico de EDS/gi,'INFORME DE TRABAJO');
  html=html.replace(/Mantenimiento electrico y mecanico de EDS/gi,'INFORME DE TRABAJO');

  // Replace the entire visible app header with the requested layout.
...
@media(max-width:600px){.head .logo{width:160px!important;height:72px!important}.head h1{font-size:20px!important}.report-brand{gap:12px!important}.report-brand img{width:115px!important;height:68px!important}.report-brand h1{font-size:19px!important}}
  ...
  if(!html.includes('id="eds-v15"')) html=html.replace('</head>',css+'</head>');
  return html;
}
